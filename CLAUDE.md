...
| `make pg_backup` | `pg_dump -Fc` of PGDATABASE into `scrap_zone/sunray-backups/`; stacks, never overwrites | — |
| `make pg_backups` | list the backups available for `pg_restore` (creates nothing) | — |
...
`[agent]` or `[user]`, and `make help` groups the targets into two sections generated from
those tags. That listing is derived from the targets themselves, so it cannot go stale — read
it rather than any list written by hand, this table included.

- **`[user]`** — the target boots, stops or upgrades the sunray server or an IMQ worker, or
  writes the developer's database. Claude must **NOT** run it. The handoff is:
...
- **`[agent]`** — safe to run unprompted: it either touches nothing (read-only/print) or works
  only in the agent's own environment.

The only exception is an explicit, in-the-moment "run it yourself" from the user — a general
past approval does not count.
...
**`update`, `test`, `initdb` and `pg_restore` now refuse to run while a unit is up** (exit 2,
`UNITS_OK=1` overrides) — these are the four a respawn corrupts: a registry lock lands mid-upgrade,
and `dropdb` reports "database is being accessed by other users". `make kill` only warns, since it
is also the deliberate way to stop a `make run-gui`. The older tell-tale still shows up if you get
there another way: `make kill` reporting `N killed, 1 still running`.
...
- Snapshot it with **`make pg_backup`** before anything risky (a `-u all`, a migration, a
  `make initdb`). It writes `<timestamp>__<database>.pg_dump` into `scrap_zone/sunray-backups/`
  via `pg_dump -Fc` (custom format, compressed), and **stacks** — a backup never overwrites a
  previous one. The server does **not** need to be stopped.
  ```bash
  make pg_backup                       # take one
  make pg_backups                      # list what is available
  make pg_restore                      # restore the most recent
  make pg_restore BACKUP=20260808-093000__cmorisse_sunray_gb_001.pg_dump
  ```
  `pg_restore` is destructive: it kills the server, drops and recreates the database. The
  backup directory is gitignored (`scrap_zone/`) and deliberately **not** matched by
