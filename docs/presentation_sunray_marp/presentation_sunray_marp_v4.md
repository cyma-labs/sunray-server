...
### 🏢 **Modèle principal — Sunray Worker FastAPI** (Middleware ForwardAuth)

Sunray agit comme un **coupe-circuit** : il **ne voit jamais le trafic applicatif**.
Votre reverse proxy interroge Sunray avant chaque requête → autorisé ou bloqué.

- **N'encaisse pas le trafic, il le filtre** — performances applicatives intactes
- Implémentation de référence : **Traefik** ; compatible avec tout proxy ForwardAuth :
  Nginx, Caddy, HAProxy, Envoy, Apache APISIX
- **Souveraineté totale** : on-premise ou cloud européen, aucune dépendance externe
- Idéal : applications sensibles, secteur public, banque, santé, conformité RGPD/NIS2

### 🌍 **Alternative — Sunray Worker for Cloudflare** (Reverse Proxy Serverless)

Pour les applications à **audience mondiale** ou lorsque le mode coupe-circuit est difficile à mettre en œuvre (pas de reverse proxy existant, déploiement edge requis).

- Reverse proxy serverless hébergé sur le réseau Cloudflare (300+ PoP mondiaux)
- Intercepte et achemine **tout le trafic** — scaling automatique, protection DDoS incluse
...
```
Client → Reverse Proxy (Traefik, Nginx, Caddy...)
              │
              ├── "est-ce autorisé ?" ──→ [Sunray Worker FastAPI]
              │                                   ↕ API
              │                             [Sunray Server]
              │                              (jamais exposé)
              │◄── autorisé / refusé ──────┘
              │
              └── si autorisé ────────────→ Application Protégée
```
...
- Pour les cas où aucun reverse proxy ForwardAuth n'est disponible, ou pour des **applications à audience mondiale**
...
4. **Souveraineté architecturale** : Le serveur décisionnaire reste dans votre infrastructure
...
### **Sunray Worker FastAPI** (Modèle principal — Middleware ForwardAuth)
- Middleware de décision d'accès (autorise / refuse / redirige)
- Compatible : Traefik, Nginx, Caddy, HAProxy, Envoy, Apache APISIX
- Cache via Redis/DragonflyDB — Déploiement Docker/Kubernetes

### **Sunray Worker Cloudflare** (Alternative — Reverse Proxy Serverless)
- Reverse proxy serverless sur réseau edge Cloudflare
- Intercepte et route toutes les requêtes
- Cache distribué via Cloudflare KV — Scaling mondial automatique
