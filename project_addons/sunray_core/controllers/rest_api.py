...
class SunrayRESTController(http.Controller):
    ...
    @http.route('/sunray-srvr/v1/email-otp/request', type='http', auth='none', methods=['POST'], cors='*', csrf=False)
    def request_email_otp(self, **kwargs):
        """Request an email OTP for authentication

        This endpoint is timing-safe: it returns the same response structure
        ...
        - client_ip: Client's IP address (optional)

        Returns:
        - {
            "success": true,
            "otp_request_id": "otp_req_...",
            "expires_at": "ISO timestamp",
            "resend_available_at": "ISO timestamp"
          }

        The response is IDENTICAL whether the email exists or not (timing-safe).
        If the email exists, an OTP code is sent via email.
        """
        # Set up request context for audit logging
        context_data = self._setup_request_context(request)
        ...
        host_domain = data.get('host_domain')
        browser_token_hash = data.get('browser_token_hash')
        client_ip = data.get('client_ip')
        ...
        if not host_domain:
            missing_fields.append('host_domain')
        if not browser_token_hash:
            missing_fields.append('browser_token_hash')

        if missing_fields:
            return self._error_response(f'Missing required fields: {", ".join(missing_fields)}', 400)

        # Find host and get config
        host_obj = request.env['sunray.host'].sudo().search([('domain', '=', host_domain)], limit=1)
        if not host_obj:
            return self._error_response('Host not found', 404)

        # Check if email login is enabled
        if not host_obj.enable_email_login:
            return self._error_response('Email login is not enabled for this host', 403)

        try:
            # Create OTP request (timing-safe)
            otp_result = request.env['sunray.email.otp'].sudo().create_email_otp(
                email=email,
                host_id=host_obj.id,
                browser_token_hash=browser_token_hash,
                client_ip=client_ip,
                user_agent=user_agent,
                validity_seconds=host_obj.email_otp_validity_s or 300
            )

            # If user exists and OTP was created, send email
            if otp_result['user_exists'] and otp_result['otp_code']:
            ...
            # Return response (same structure regardless of user existence)
            # Note: browser_token is generated by worker, not returned here
            return self._json_response({
                'success': True,
                'otp_request_id': otp_result['otp_request_id'],
                'expires_at': otp_result['expires_at'].isoformat(),
                'resend_available_at': otp_result['resend_available_at'].isoformat()
            })

        except Exception as e:
            _logger.error(f"Unexpected error in email OTP request: {str(e)}")

            # Create audit event for system error
            request.env['sunray.audit.log'].sudo().create_audit_event(
                event_type='auth.email_otp_system_error',
                details={
                    'email': email,
                    'host_domain': host_domain,
                    'error': str(e),
    ...
    def _send_otp_email(self, email, otp_code, host_obj, client_ip, validity_seconds):
        """Send OTP code via email using Odoo mail template.
        ...
        Args:
            ...
            otp_code: Plain OTP code (e.g., 'A2B3-C4D5')
            host_obj: Protected host record
            client_ip: Client IP address for email context
            validity_seconds: OTP validity in seconds
        """
        ...
        try:
            ...
            template.sudo().with_context(
                ...
                host_obj.id,
                force_send=True,
                raise_exception=False  # Don't raise for timing safety
            )

        except Exception as e:
            _logger.error(f"Failed to send OTP email to {email}: {str(e)}")
            # Don't raise - maintain timing-safe behavior

    @http.route('/sunray-srvr/v1/email-otp/validate', type='http', auth='none', methods=['POST'], cors='*', csrf=False)
    def validate_email_otp(self, **kwargs):
        ...
        JSON Body:
        - email: User's email address
        - otp_code: OTP code entered by user (format: AAAA-BBBB)
        - otp_request_id: OTP request identifier
        - browser_token_hash: SHA-256 hash of browser token from cookie
        - host_domain: Domain being protected
        - client_ip: Client's IP address (optional)

        Returns:
        - {"valid": true, "user_id": 42, "username": "jdoe", "session_duration_s": 3600} on success
        ...
        if not otp_request_id:
            missing_fields.append('otp_request_id')
        if not browser_token_hash:
            missing_fields.append('browser_token_hash')
        if not host_domain:
            missing_fields.append('host_domain')
        ...
        # Get host config for max_attempts
        host_obj = request.env['sunray.host'].sudo().search([('domain', '=', host_domain)], limit=1)
        max_attempts = host_obj.email_otp_max_attempts if host_obj else 5

        try:
            # Validate OTP (browser_token_hash already computed by worker)
            validation_result = request.env['sunray.email.otp'].sudo().validate_email_otp(
                email=email,
                otp_code=otp_code,
            ...
            if validation_result['valid']:
                return self._json_response({
                    ...
                    'session_duration_s': validation_result['session_duration_s']
                })
            else:
                return self._json_response({'valid': False})

        except Exception as e:
            _logger.error(f"Unexpected error in email OTP validation: {str(e)}")
