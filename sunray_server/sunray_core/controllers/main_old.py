...
class SunrayAPIController(http.Controller):
    ...
    def get_status(self, **kwargs):
        """Health check endpoint - no authentication required for basic status"""
        ...
        ip_info = {
            ...
            'cf_connecting_ip': headers.get('CF-Connecting-IP'),  # Real client IP from Cloudflare
            'cf_ipcountry': headers.get('CF-IPCountry'),  # Country code
            'cf_ray': headers.get('CF-RAY'),  # Cloudflare Ray ID for debugging
            'cf_visitor': headers.get('CF-Visitor'),  # Protocol info
            
            # Cloudflared tunnel headers
            'cf_access_authenticated_user_email': headers.get('CF-Access-Authenticated-User-Email'),
            'cf_access_jwt_assertion': headers.get('CF-Access-JWT-Assertion') and 'present',  # Don't log full JWT
        ...
        return Response(
            json_lib.dumps(status_data, indent=2),
            content_type='application/json',
            status=200
        )
    
    @http.route('/sunray-srvr/v1/health', type='json', auth='none', methods=['GET'], cors='*')
    def health_check(self, **kwargs):
        """Detailed health check with authentication"""
        # Basic health without auth
        health = {
            'status': 'healthy',
            'timestamp': fields.Datetime.now().isoformat()
        }
